...
void main() {
  ...
  // The broken pieces and the light spilling past the shell are drawn by
  // shaders/shards.frag over the whole screen. They used to be here, and were
  // cut off wherever this box ended.
  if (alpha <= 0.0) {
    fragColor = vec4(0.0);
    return;
  }
